...
const mockGetPosts = (userId: string) => {
  return Promise.resolve([
    { title: "An Example Post", content: "Hello from Clerk + Next.js", userId },
  ]);
};
...
const SSRDemoPage = ({ posts }: { posts: Post[] }) => {
      ...
      <main className={styles.main}>
        ...
        <div className={styles.preContainer}>
          ...
          <p className={styles.instructions}>
            `<strong>getServerSideProps</strong>` uses `<strong>getAuth</strong>
            ` to get the userId and fetch the posts from a remote database
          </p>
          <pre>
        ...
        <div className={styles.preContainer}>
          ...
          <p className={styles.instructions}>
            Using <strong>{`clerkClient.users.getUser`}</strong> during the SSR
            request allows all Clerk data available both during SSR and CSR
          </p>
          <pre>
